...
| Screenshots | native res, ≤5 per platform, PNG/GIF | `docs/screenshots/<platform>_*.png` | ✅ 5 maneuvers × 4 platforms (20) |

Notes:
- Maneuver screenshots ship for `basalt_*` (144×168), `chalk_*` (180×180, round),
  `diorite_*` (144×168, b/w) and `emery_*` (200×228). Each set is
  left / right / roundabout / straight / arrive. The background red is recoloured to
  the mockup's `#ff313e` (basalt/chalk/emery; diorite is b/w).
...
- basalt/chalk/diorite/emery were generated by injecting `NAV_TURN` + `NAV_TEXT` + `NAV_ETA`
...
5. Upload **screenshots** per platform from `docs/screenshots/` (basalt, chalk,
   diorite, emery — 5 each), then paste the **description** and **release notes**.
...
- 🟡 **aplite deferred to 1.1.** The classic-Pebble build links with only ~624 bytes
  of free heap and crashes on launch (PDC icons + transition buffers overflow it), so
  the app never foregrounds. aplite was dropped from `targetPlatforms` for 1.0.0; to
  bring it back in 1.1, slim the aplite build (e.g. shrink `ICON_PTS_CAP`, disable the
  animated transition layer on aplite, or use lighter icons).
- ✅ **emery store screenshots captured** (via the SDK 4.9.77 emulator; see the
  emery capture note in section 2). All 4 shipping platforms now have 5 frames each.
